...
var app = {
	xreaderCalls : {
		Inited : function(params) {
			...
			xreader.Environment.AppVersion = "2.0.1";
...
XReader.JS Version ${xreader.Environment.AppVersion}`, "font-family: monospace; color: #fc546f;");
	...
	startReadaloud : function(page) {
		...
		if (player.src != src || player.paused) {
			...
			xreader.applyMediaOverlay(self.readaloudIsPlaying, null, null);
		}

		xreader.applyMediaOverlay(self.readaloudIsPlaying, app.rendition.contentsUrl + page.url, self.readaloudCurrentBlock != null ? self.readaloudCurrentBlock.DOMElementId : null);
		...
		if (self.readaloudInterval == null) {
			self.readaloudInterval = setInterval(function() {
				...
				if (block != self.readaloudCurrentBlock) {
					...
					xreader.applyMediaOverlay(self.readaloudIsPlaying, app.rendition.contentsUrl + page.url, block != null ? block.DOMElementId : null);
				}
			}, 100);
		}
	},
	stopReadaloud : function() {
		...
		xreader.applyMediaOverlay(false, null, null);
	},
