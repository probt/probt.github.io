...
var xreader = window.PubViewer || {
	...
	onWheel: function (ev, iframeEvent) {
		...
		// console.warn("wheeling!", e.deltaX, xreader.accWheelX, ms);
		...
		if (Math.abs(xreader.accWheelX) > 500) {
	...
	initReaderPage: function(iframeElement, src, pagePageInstanceId) {
		...
		if (xreader.Environment.Platform != xreader.Platform.Web) iframeElement.contentWindow.xreader = xreader;
